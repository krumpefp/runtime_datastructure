...
impl Root {
    ...
    pub fn init_PST_3D(mut data : &mut Vec<Root>) -> usize {
...
fn create_root_x(mut root_refs: Vec<RootRef>, mut data: &mut Vec<Root>) -> usize {
    ...
    let mut root_idx = find_root_idx(&mut root_refs);
    ...
    if root_refs.len() == 1 {
        ...
        if median_idx >= root_refs.len() {
            ...
            let mut last = root_refs.split_off(median_idx);
    ...
    let r = data.get_mut(root_idx).expect("Trying to access element at not existing vector position");
